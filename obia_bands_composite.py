...
def obia(band1,band2,band3, dst_file,proj, gt):
    ...
    #band,proj,gt=read_raster(imge_path)
    bands_data = []
...
def seg_raster(rasterfile):
    ...
    for i in range(rnum_tile):
        ...
        for j in range(cnum_tile):
            ...
            dst_file=os.path.join(seg_path,image[0:22]+str(i)+'_'+str(j)+'.tif')
            gt[0]=geotrans[0]+xoff*geotrans[1]
            gt[3]=geotrans[3]+yoff*geotrans[5]
            
            subtask_list.append(image[0:22]+str(i)+'_'+str(j))
            ...
            obia(band1,band2,band3,dst_file,proj, gt)
    bb = {}
